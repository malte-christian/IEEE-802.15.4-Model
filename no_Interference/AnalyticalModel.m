function [result] = AnalyticalModel()
    ...
    % Frame delay
    TFrame = @(x, RData, LAddress) 8 * (LPhy + LMac_Hdr + LAddress + x + LMac_Ftr) / RData;
    ...
    % Delay in total
    delay = @(x, RData, TS, LAddress) (TBo + TFrame(x, RData, LAddress) + TTa(TS) + TAck(RData) + TIfs(x, TS, LAddress)) * checkSize(x, LAddress);
